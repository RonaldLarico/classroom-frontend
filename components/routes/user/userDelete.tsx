...
const UserDelete: React.FC<UserDeleteProps> = ({ userId, onDeleteSuccess }) => {
  ...
  const handleDelete = async () => {
    try {
      if (userId) {
        await axios.delete(`${URL()}/student/${userId}`, tokenConfig(validToken));
        onDeleteSuccess();
      }
    } catch (error) {
    ...
    <>
      <div>
        <button
          ...
          className='border-2 border-red-500 p-0.5 rounded-md text-red-500 transition ease-in-out delay-300 hover:scale-125 duration-300'
        >
      ...
      {confirmationModalOpen && (
        <Modal open={confirmationModalOpen} onClose={closeModal}>
          <div className='border p-5 rounded-lg'>
            <p className='flex justify-center whitespace-pre-wrap text-center text-primary-color'>¿Estás seguro de que deseas eliminar a este admin?</p>
